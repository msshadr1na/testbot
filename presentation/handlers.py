...
# --- Клиент (бот): организация, расписание, записи, веб -----------------------
...
async def _send_client_schedule(message: Message, state: FSMContext):
    ...
    if not rows:
        await message.answer("На ближайшие 7 дней нет тренировок в расписании.")
        return
...
async def client_pick_org_handler(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        ...
        "• /bookings — ваши записи на тренировки\n"
        "• /exit — выход в главное меню\n"
        "• кнопка «Управление в веб-приложении» — кабинет клиента в этой организации",
        reply_markup=None,
    )
...
async def worker_open_webapp(callback: CallbackQuery):
    url = f"{web_app_base_url}/app/worker_orgs.html"
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="Открыть кабинет тренера", web_app=WebAppInfo(url=url))],
            [InlineKeyboardButton(text="Назад", callback_data="start")],
        ]
    )
    await callback.message.edit_text(
        "Кабинет работника доступен в веб-приложении.",
        reply_markup=kb,
    )
    await callback.answer()
...
@router.message(F.text == "◀️ Назад в меню", StateFilter(UserState.client_menu))
...
async def cancel_delete_org(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    keyboard = presentation.keyboards.build_start_keyboard()
